...
import './assets/CustomTannyTable.css'
import { useTable, useSortBy } from 'react-table'

const CustomTannyTable = ({
  typeId,
  data,
  columns,
  insertFunction,
    ...
    useTable(
      {
        columns,
        data: data || [],
        autoResetSortBy: false //DONT REMOVE THIS IT CAUSES UNEXPLAINED BUGSs
      },
  ...
  const handleEditSaveClick = () => {
    setAddingMode(false)
    setEditingMode(false)
    console.log('edit mode', activeRows)
  ...
  return (
    <div className="CustomTannyTable">
      <div className="CustomTannyTable__button">
        <button
          onKeyDown={(event) => {
            if (event.key === 'Escape') {
              handleAddClick()
      ...
      <table {...getTableProps()} className="CustomTannyTable__table">
        <thead className="CustomTannyTable__tableHeader">
          {headerGroups.map((headerGroup) => (
            <tr {...headerGroup.getHeaderGroupProps()} className="CustomTannyTable__tableRow">
              {headerGroup.headers.map((column) => (
                <th
                  {...column.getHeaderProps(column.getSortByToggleProps())}
                  className="CustomTannyTable__tableHeaderCell"
                >
                  {column.render('Header')}
                  {/* Add a sort direction indicator */}
                  <span className="CustomTannyTable__sortIndicator">
        ...
        <tbody {...getTableBodyProps()} className="CustomTannyTable__tableBody">
          {addingMode && (
            <tr className="CustomTannyTable__tableRow newRow">
              <td className="CustomTannyTable__tableCell"></td>
              <td className="CustomTannyTable__tableCell"></td>
              <td className="CustomTannyTable__tableCell">
          ...
          {rows.map((row) => {
            ...
            return (
              <tr
                ...
                className="CustomTannyTable__tableRow"
              >
                {row.cells.map((cell) => (
                  <td
                    onClick={() => {
                    ...
                    {...cell.getCellProps()}
                    className="CustomTannyTable__tableCell"
                  >
                    {cell.render('Cell', {
        ...
        <tfoot
          className="CustomTannyTable__tableFooter"
          style={{
            background: '#55608f'
          }}
        >
          {footerGroups.map((group) => (
            <tr className="CustomTannyTable__tableFooter--Row" {...group.getFooterGroupProps()}>
